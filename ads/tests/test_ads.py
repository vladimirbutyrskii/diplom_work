...
class TestAdUpdate:
    ...
    def test_update_any_ad_by_admin(self, admin_client, ad):
        ...
        response = admin_client.patch(
            url, {"title": "Отредактировано админом"}, format="json"
        )
...
class TestAdSearch:
    ...
    def setup_ads(self, user):
        ...
        Ad.objects.create(
            author=user, title="Продам гараж", price=50000, description="Кирпичный"
        )
        Ad.objects.create(
            author=user, title="Куплю автомобиль", price=100000, description="Срочно"
        )
        Ad.objects.create(
            author=user,
            title="Сдам квартиру",
...
class TestCommentUpdate:
    ...
    def test_update_any_comment_by_admin(self, admin_client, ad, comment):
        ...
        url = reverse("ad-comments-detail", args=[ad.id, comment.id])
        response = admin_client.patch(
            url, {"text": "Отредактировано админом"}, format="json"
        )
